...
### 4b.1 Celery + Redis (evaluado y revertido)

> Se implemento el stack completo de procesamiento asincrono como ejercicio de
> aprendizaje (Stack Orpheus, feb 2026): Celery + Redis + Docker Compose con
> 6 servicios (web, db, redis, worker, beat, flower).
>
> **Decision final: revertido.** El volumen de emails transaccionales (<50/mes)
> no justifica la complejidad operativa adicional (Redis, worker, beat, monitoring).
> Las notificaciones se ejecutan de forma sincrona desde `notifications.py` via
> Brevo SMTP (<1s por email). Si el volumen crece significativamente en el futuro,
> el conocimiento adquirido permite reintroducir Celery con confianza.
>
> El codigo fue comiteado, probado y revertido. No quedan trazas en el repositorio.
...
5. **Cola de tareas**: Celery + Redis para emails asíncronos (evaluado y revertido, ver sección 4b)
